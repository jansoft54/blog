...
Note that the above formulation of $$R^d_{\Theta,m}$$ is highly inefficient, as it constructs a mostly empty $$d \times d$$ matrix. Later, when we implement $$\textbf{RoPE}$$ into the Llama model, we will explore a more efficient method to perform this 2D subvector rotation.
