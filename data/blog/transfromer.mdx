---
title: Introduction to Transformers - Part 01
date: '2024-08-21'
tags: ['theory']
draft: false
summary: A short introduction to theory behind transformers how they can be utilized in NLP tasks.
---
...
An example of such plots for different covariance matrices $\Sigma$ is shown in the figure below, demonstrating how the covariance influences the relationships between variables.
...
functions that pass through our training points (as shown in the image above) and are therefore able
to explain the distribution of our data.
...
The figure also illustrates the **95% confidence interval** ($$2\sigma$$) and the mean function.
...
where the second equality stems from the fact that we assume $$\mu_{X} = 0$$ and $$\mu_{Y}=0$$.
